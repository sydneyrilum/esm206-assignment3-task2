...
**Note: You should get a warning about one piece dropped. What happens to the name in Row 49, which contains two dashes? (the last part of the name disappears)

```{r}
# Your code here: 
sw_split_name <- starwars %>% # Start with starwars
  separate(col = name, # Separate the name column...
           into = c("name_1","name_2"), # Into two columns
...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "CAT!" # Puts "HEY!" between the pieces (default is underscore)
        ) 
...
gasol_hookshots_month <- gasol_hookshots %>% 
  count(month) 
...
storms_danger <- storms %>%
  mutate(danger = case_when(status == "hurricane" ~ "high",
                            TRUE ~ "moderate"))
```
